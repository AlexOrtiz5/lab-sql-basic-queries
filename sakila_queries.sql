...
FROM
    staff;
-- 5.3 Determine how many films are available for rent and how many 
-- have been rented.
SELECT 
    COUNT(DISTINCT film_id) AS available_for_rent
FROM
    inventory;
SELECT 
    COUNT(DISTINCT rental_id) AS rented
FROM
    rental;
-- 5.4 Determine the number of distinct last names of the actors in 
-- the database.
SELECT 
...
SELECT 
    COUNT(special_features) AS behind_the_scenes
FROM
    film
WHERE
